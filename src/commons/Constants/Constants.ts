export const PATH_DIALOGS = '/dialogs';
export const PATH_CHAT = '/chat';
export const PATH_USERS = '/users';
export const PATH_LOGIN = '/login';


export const WS_WSS_URL = 'wss://social-network.samuraijs.com/handlers/ChatHandler.ashx';
export const WS_CHANNEL_STATUS_PENDING = 'pending';
export const WS_CHANNEL_STATUS_READY = 'ready';
export const WS_CHANNEL_EVENT_CLOSE = 'close';
export const WS_CHANNEL_EVENT_MESSAGE = 'message';
export const WS_CHANNEL_EVENT_OPEN = 'open';
export const WS_CHANNEL_EVENT_ERROR = 'error';